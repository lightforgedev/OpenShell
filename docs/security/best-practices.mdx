...
<Note>
UID/GID-scoped binary rules separate agent users inside one organization sandbox.
They do not make multiple customer organizations safe in one shared Linux namespace.
Per-agent separation assumes sandbox processes cannot use setuid/setgid binaries or `CAP_SETUID`/`CAP_SETGID` to present another effective identity.
For the nested-organization design analysis, see [Nested Org Sandbox Feasibility](/security/nested-org-sandbox-feasibility).
...
The proxy identifies which process initiated each connection by reading `/proc/<pid>/exe` (the kernel-trusted executable path) and `/proc/<pid>/status` for effective UID/GID.
...
| What you can change | Add binaries to an endpoint entry. Use glob patterns for directory-scoped access (for example, `/sandbox/.vscode-server/**`). Add `uid` and/or `gid` to bind a grant to one effective OS user/group inside a shared sandbox. |
...
| Recommendation | Scope binaries to the specific executables that need each endpoint. In multi-agent sandboxes, also scope shared tool paths by agent UID/GID. Use narrow globs when the exact path varies (for example, across Python virtual environments). |
