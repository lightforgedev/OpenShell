...
## Shorthand Severity Threshold

The shorthand OCSF layer emits every OCSF event when `ocsf_shorthand_min_severity` is unset. Set the runtime setting to reduce container-log noise without changing enforcement or JSONL audit capture:

```shell
openshell settings set --global --key ocsf_shorthand_min_severity --value medium
```

Allowed values are `informational`, `low`, `medium`, `high`, `critical`, and `fatal`. The setting can be global or per-sandbox and takes effect on the next settings poll cycle; no sandbox restart is required. Gateway TOML can set the global value:
...
The TOML value is reapplied at every gateway startup and overwrites runtime
`openshell settings set --global` changes to the same key.

With `medium`, routine successful OCSF events such as `HTTP:POST [INFO]` are suppressed from shorthand logs, while denials and findings remain visible.
...
When the supervisor cannot read `/proc/<pid>/root/<binary>` for symlink resolution, binary policy matching falls back to the literal process path and emits a warning. If a policy names a symlink such as `/usr/bin/python3`, either use the canonical target path in policy or run the supervisor with the capability needed to read the process root, such as `CAP_SYS_PTRACE` in Linux container deployments.
