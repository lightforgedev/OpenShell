...
Allowed values are `informational`, `low`, `medium`, `high`, `critical`, and `fatal`. The setting can be global or per-sandbox and takes effect on the next settings poll cycle; no sandbox restart is required. Gateway TOML can seed the global default:
