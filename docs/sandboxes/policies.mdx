...
Binary entries may include `uid` and/or `gid` to bind the grant to the socket owner's effective process identity. Omit both fields for legacy sandbox-wide binary matching.
