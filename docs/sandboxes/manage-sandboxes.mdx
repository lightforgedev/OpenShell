...
Run a command as a non-root OS user that exists inside the sandbox:

```shell
openshell sandbox exec -n my-sandbox --user ubuntu -- id
```

Use `--user` when a shared sandbox has multiple agent OS users and policy binds network grants to UID/GID. Root (`root` or `0`) is rejected.
...
| `--user`       | Run the command as a non-root OS user inside the sandbox. |
