...
<<<<<<< HEAD
The file is rooted at `[openshell]`. Gateway-wide process settings live under `[openshell.gateway]`. Runtime settings seeded into the gateway settings store live under `[openshell.settings]`. Each compute driver owns its own `[openshell.drivers.<name>]` table. Shared keys set at gateway scope are inherited into driver tables when not overridden.
=======
...
[openshell.settings]
# Suppress routine successful OCSF shorthand events in container logs while
# keeping denials/findings visible. Full JSONL export is controlled separately
# by ocsf_json_enabled.
...
`[openshell.settings]` accepts registered runtime setting keys and applies them as global settings at every gateway startup. TOML is authoritative on restart: it overwrites runtime `openshell settings set --global` changes to the same keys. Unknown keys and wrong TOML value types are rejected during startup. These settings use the same validation and precedence as `openshell settings set --global`: gateway-global values override sandbox-scoped values.
...
# Containerized gateways using the host Docker socket should replace both
# supervisor_bin and supervisor_image with a digest-pinned daemon image mount:
# supervisor_image_mount = "ghcr.io/nvidia/openshell/supervisor@sha256:<digest>"
guest_tls_ca       = "/etc/openshell/certs/ca.pem"
guest_tls_cert     = "/etc/openshell/certs/client.pem"
guest_tls_key      = "/etc/openshell/certs/client-key.pem"
